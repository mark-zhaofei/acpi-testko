 ...
 *
 */
void mdm_ctrl_enable_flashing(struct timer_list *param)
{
	struct mdm_info *mdm = (struct mdm_info *)
                       container_of(param, struct mdm_info, flashing_timer);
...
int mdm_ctrl_set_pmic(struct mdm_info *mdm)
{
	...
	if (pdata->pmic_data) {
		...
		case MODEM_2230:
			...
			pdata->pmic.power_on_mdm = NULL;
			pdata->pmic.power_off_mdm = NULL;
			break;
		default:
			break;
		}
