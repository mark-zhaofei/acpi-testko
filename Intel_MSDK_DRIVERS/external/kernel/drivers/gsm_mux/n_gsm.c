...
static int debug = 0x3F;
module_param(debug, int, 0600);

#define MAX_CONFIG_LEN 256
#define MAX_BUF_SIZE  (9 * 65536)
		...
		if (dlci->retries) {
			...
			if (debug & 8)
				pr_info("DLCI %d opening in ADM mode.\n",
					dlci->addr);
			pr_debug("try gsm_dlci_open\n");
			gsm_dlci_open(dlci);
		} else {
			pr_debug("try gsm_dlci_close\n");
			gsm_dlci_close(dlci);
		}
...
/**
 *	gsm_closeall_dlci		-	generic GSM protocol cleanup
 *	@gsm: our mux
 *
...
static int gsm_add_one_mux_entry(struct gsm_mux *gsm)
{
	...
	if (mux_base_config[0]) {
		...
		for (i = 0; i < MAX_MUX; i++) {
			if (gsm_mux[i] == NULL) {
				gsm->num = i;
				pr_debug("gsm_add_one_mux_entry[%d] = %s\n", i, gsm ? "not null" : "null");
				gsm_mux[i] = gsm;
				break;
			}
			else{
				pr_debug("gsm_add_one_mux_entry[%d] not null\n", i);
			}
		}
		spin_unlock(&gsm_mux_lock);
	}
...
static int gsmld_attach_gsm(struct tty_struct *tty, struct gsm_mux *gsm)
{
	...
	if (ret != 0)
		...
		pr_debug("gsmld_attach_gsm\n");
		base = gsm->num << 6; /* Base for this MUX */
		for (i = 1; i < NUM_DLCI; i++){
			pr_debug("gsmld_attach_gsm - %d\n", base + i);
...
static int gsmld_open(struct tty_struct *tty)
{
	...
	if (ret != 0) {
		pr_debug("gsmld_attach_gsm failed\n");
		gsm_cleanup_mux(gsm);
		mux_put(gsm);
	}
	return ret;
							...
							struct gsm_config *c)
{
	...
	if (need_close || need_restart) {
		...
		pr_debug("gsmld_config need_close || need_restart\n");
	...
	if (need_restart) {
		pr_debug("gsmld_config need_restart\n");
		gsm_cleanup_mux(gsm);
		wake_up(&gsm->event);
    }
	...
	if (need_restart){
		pr_debug("gsmld_config need_restart -> __gsm_activate_mux\n");
		//__gsm_activate_mux(gsm);
		gsm_activate_mux(gsm);
	}
	if (gsm->initiator && need_close){
		pr_debug("gsmld_config need_close -> gsm_dlci_begin_open\n");
		       ...
		       unsigned int cmd, unsigned long arg)
{
	...
	struct gsm_mux *gsm = tty->disc_data;
        ...
        pr_debug("gsmld_ioctl -->\n");
	switch (cmd) {
	case GSMIOC_GETCONF:
		pr_debug("gsmld_ioctl GSMIOC_GETCONF \n");
		memset(&c, 0, sizeof(c));
		...
		return 0;
	case GSMIOC_SETCONF:
		pr_debug("gsmld_ioctl GSMIOC_SETCONF \n");
		if (copy_from_user(&c, (void *)arg, sizeof(c)))
			return -EFAULT;
		return gsmld_config(tty, gsm, &c);
	case GSMIOC_DEMUX:
		pr_debug("gsmld_ioctl GSMIOC_DEMUX \n");
		gsm->dead = 1;
		gsm_closeall_dlci(gsm);
		return 0;
	default:
		pr_debug("gsmld_ioctl default\n");
...
static int gsmtty_chars_in_buffer(struct tty_struct *tty)
{
	...
	pr_debug("gsmtty_chars_in_buffer\n");
	if (!tty)
		return 0;
...
static int mux_base_conf_set(const char *kmessage, const struct kernel_param *kp)
{
